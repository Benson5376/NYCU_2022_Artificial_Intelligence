...
class Agent():
    ...
    def learn(self):
        ...
        #----------2. Sample trajectories of batch size from the replay buffer----------
        _sample_ = replay_buffer.sample(self.buffer,self.batch_size)
        # <class 'tuple'> 
        # (([state]...),(1,1,0...),(1.0,1.0,...),([next_state]...),(0,0,1...))
        
        s_list = list(_sample_[4])  
        for i in range(32):
           s_list[i] = 1 - s_list[i] 
        _done_ = tuple(s_list)
        
        # For example, 1,0,0,1,0 turn into
        # 0,1,1,0,1 if "done", multiple 0 for q value
        
        _state_ = torch.FloatTensor(_sample_[0])
        _action_ = torch.LongTensor(_sample_[1])
        _reward_ = torch.FloatTensor(_sample_[2])
        _next_state_ = torch.FloatTensor(_sample_[3])
        _done_ = torch.FloatTensor(_done_)

        _action_ = torch.unsqueeze(_action_ , 1)
        _reward_ = torch.unsqueeze(_reward_ , 1)
        
        #----------3. Forward the data to the evaluate net and the target net.----------
        q_evaluate = self.evaluate_net(_state_).gather(1 , _action_)
        q_next = self.target_net(_next_state_).detach()
        q_next = q_next.max(1)[0]
        
        Q = torch.mul(q_next , _done_)
        q_target = _reward_ + self.gamma * Q.view(self.batch_size, 1)
        
        #----------4. Compute the loss with MSE.----------
        lossfunc = nn.MSELoss()
        loss = lossfunc(q_evaluate, q_target)
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            # Begin your code
            t = torch.unsqueeze(torch.FloatTensor(state), 0)
            
            expexp = np.random.random_sample()
            
            if expexp > self.epsilon:
                q_value = self.evaluate_net(t) 
    ...
    def check_max_Q(self):
        ...
        max_q = torch.max(self.evaluate_net.forward(initial))
        return max_q
        # End your code
