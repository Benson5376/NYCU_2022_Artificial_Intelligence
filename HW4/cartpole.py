...
class Agent():
    ...
    def discretize_value(self, value, bins):
        ...
        digitized = np.digitize( x = value , bins = bins)
        return digitized
    ...
    def discretize_observation(self, observation):
        ...
        A = self.init_bins(-2.4, 2.4, self.num_bins)
        B = self.init_bins(-3.0, 3.0, self.num_bins)
        C = self.init_bins(-0.5, 0.5, self.num_bins)
        D = self.init_bins(-2.0, 2.0, self.num_bins)
        a = self.discretize_value(x , A)
        b = self.discretize_value(v1 , B)
        c = self.discretize_value(theta , C)
        d = self.discretize_value(v2 , D)
    ...
    def choose_action(self, state):
        ...
        if expexp > self.epsilon:
            action = np.argmax(self.qtable[a,b,c,d])
        else :
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        # End your code
        # print(self.qtable[state[0],state[1],state[2],state[3],action])
        # You can add some conditions to decide when to save your table
        if(self.qtable[state[0],state[1],state[2],state[3],action] >= 33):
            reward += 2
        if(self.qtable[state[0],state[1],state[2],state[3],action] >= 28):
    ...
    def check_max_Q(self):
        ...
        state = self.discretize_observation(initial_observation)
        max_q = np.max(self.qtable[state[0],state[1],state[2],state[3]])
        return max_q
        # End your code
