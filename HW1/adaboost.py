...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        F = np.zeros((len(features),len(iis)))
                    
   
        for i in range(len(features)):
            for j in range(len(iis)):
            
                if(featureVals[i][j] < -10):
                    F[i][j] = 1
                elif(featureVals[i][j] < 50):
                    F[i][j] = 0.2
                else:
                    F[i][j] = 0
         
                error[i] = error[i] + weights[j] * abs(F[i][j] - labels[j])
        ...
        bestClf = WeakClassifier(features[fid])
